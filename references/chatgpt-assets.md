...
|---|---|---|
| 1 | 每角色·脸部白底三视图（定妆图） | —（链的源头；不满意就重来，后面全靠它）。提示词含 §3 的**性格外化**（站姿/体态/默认神态按性格关键词） |
| 2 | 每角色·情感表情图（≥6 表情一张网格） | 该角色定妆图。**6 表情按 §3 表情清单**（剧本实际情绪+表演方式定强度），不要通用喜怒哀乐套餐 |
| 3 | 每套服装·白底三视图 | —（纯服装，无人） |
| 4 | 每角色×每套服装·着装三视图 | 定妆图 + 对应服装图 **两张都挂**。姿态延续性格外化；§3 有**标志性小动作**的，正面视图摆出该动作（如惯性摸表带/抱臂） |
| 5 | 每场景×4 方向（东南西北/前后左右） | 同场景已生成的方向图（第 2–4 张挂第 1 张，锁空间一致） |
| 6 | 主要道具·白底三视图 | —（对照 §13 描述核对细节）。**逐一对照 §13 清单生成全部主要道具，不得静默裁剪**——省略任何一个需用户批准并记 state.md |
