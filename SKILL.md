...
2b. **视频提示词一律中文「四框架」格式**（2026-07 星火定稿，取代旧十区罗列）：【整体规格】(场景/风格/色彩/情绪/主体+{{Node}}绑定/群演) → 【对白使用原则】(仅有台词时) → 【镜头】(逐行 `镜头N(a-b s):运镜|内容`) → 【约束】(负面+本镜特殊注意)。由 GPT 在 §17 直接产出成品，**不翻译不压缩原样喂给视频模型**；中文关键词信息密度约为英文散文 6 倍；Seedance `maxLength: 0` 无长度上限（**只有 Mureka 是 1024**）。**镜头行细节标准**：动作链用具体动词串联、物品移动写起点→终点→方式、**特写镜头必写眉/眼/嘴微表情变化**、在场者逐一点名（配 2d 排除法）；**动作与表情从 §3 角色小传推导**——体现该角色肢体习惯/标志性小动作，表达强度按其表演方式定，同一情绪不同角色演法必须不同；【对白使用原则】的语气行引用 §3 表演方式（语速/音色/说话习惯）；**镜头语言从 §7/§8/§10 推导**——焦段/机位/运镜从 §8 场景×镜头搭配表取值、运镜动词在 §10 词表内、不违反 §7 可执行规则，同场景镜头语言成套；**运动主体必加物理真实关键词**——真实运动感、速度感、重力感、惯性感、符合真实阻力逻辑（按介质具体化），复杂载具运动升级用运动四宫格挂参考。格式全文与范例见 references/production-plan-template.md §17。
